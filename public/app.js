...
function generateBlockchainRecordsHTML() {
    ...
    sortedRecords.slice(0, 20).forEach(record => {
        // FIX: Update these field mappings
        const recordType = record.recordType || 'unknown';
        const displayType = recordType.replace('_', ' ').toUpperCase();
        const recordHash = record.dataHash || record.merkleRoot || 'undefined';
        const description = `${recordType} record created`;
        ...
        recordsHTML += `
            <div class="blockchain-record">
                ...
                <div class="record-details">
                    <p><strong>Hash:</strong> <code class="blockchain-hash" onclick="copyToClipboard('${recordHash}')">${recordHash}</code></p>
                ...
                <div class="record-actions">
                    ...
                    <button class="btn btn-sm btn-outline" onclick="copyToClipboard('${recordHash}')">Copy Hash</button>
                </div>
            </div>
        `;
    });
...
async function verifyRecord(event) {
    ...
    try {
        ...
        if (result.success && result.verified) {
            let message = '✅ Record verified successfully!';
            if (result.contractVerified) {
                message += ` (Sepolia blockchain confirmed)`;
            } else {
                message += ` (Local database only)`;
            }
            showMessage(message, 'success');
        } else {
            showMessage(`❌ Verification failed: ${result.error || 'Record not found'}`, 'error');
        }
    } catch (error) {
